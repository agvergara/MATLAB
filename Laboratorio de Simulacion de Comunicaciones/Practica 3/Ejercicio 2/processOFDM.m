function [dataFFT, y, BER] = processOFDM(channel, PC, N, simbOFDM, modData, mod, bits, ganancias)
...
canalPar = reshape(ganancias,PC+N,ceil(length(simbOFDM)/(PC+N)));
...
h_mean = mean(canalNPC); %Debemos coger un valor que represente a todo el canal, se ha optado por la media
h_elim = fft([h_mean;zeros(size(h_mean))],N); %Se pone una fila de ceros para que la fft se realize por columnas
y = dataFFT./h_elim;
